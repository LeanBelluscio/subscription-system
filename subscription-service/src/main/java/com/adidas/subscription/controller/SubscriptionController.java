...
public class SubscriptionController {
	...
	SubscriptionResponse createSubscription(@Valid @RequestBody SubscriptionRequest body, @RequestHeader("Authorization") String accessToken) throws ObjectNotFoundException, SubscriptionUniqueErrorException, EmailServiceErrorException, Exception {
		logger.info("Creating new subscription");
		Subscription subscription =  this.subscriptionService.createNewSubscription(body);
		SubscriptionResponse response = new SubscriptionResponse();
		emailApiConsumer.sendSubscriptionEmail(body, accessToken);
